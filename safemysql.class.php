 ...
 * ?h ("string")  - strings Filtered <script> <iframe> tags
...
class SafeMySQL {
    ...
    private $defaults = array(
        ...
        'pconnect' => FALSE,
        'charset' => 'utf8',
        'errmode' => 'error', // error or logfile
    ...
    private function prepareQuery($args) {
        ...
        $array = preg_split('~(\?[nsiuapfh])~u', $raw, null, PREG_SPLIT_DELIM_CAPTURE);
        $anum = count($args);
    ...
    private function escapeFloat($value) {
        ...
        if (!is_numeric($value)) {
            $this->error("Float (?f) placeholder expects numeric value, " . gettype($value) . " given");
            return FALSE;
        }
        return number_format($value, 7, '.', '') * 1;
    }
